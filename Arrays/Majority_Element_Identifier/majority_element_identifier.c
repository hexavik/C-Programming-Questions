...
/**
 * @brief Calculate the presence of majority element, only if the
 * number of its occurrence is greater than the half lenght of the
 * input array. We use hash map to caluclate the frequency of an array
 * elements and at the same time we shall keep a track on maximum
 * frequency element.
 *
 ...
 */
int hash_repeat_calculator(int arr[], size_t length) {
    // Create a hash table assuming array element values are within
    // 0 - 1000 range
